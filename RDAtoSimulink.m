%% ***********************************************************************   
% Modified by Juliette Ngan - May 2021
% Use RDA to record EMG data and send to simulink through udp connection 
% Ensure simulink model runs in other instance of Matlab 
% TODO: automate parallel computing so no need to start model manually 
%% **********************************************************************
...
connection = pnet('tcpconnect', recorderip, 51244);
...
function EEGdata = RDA(con)
   EEGdata = [];
    % Change for individual recorder host
    persistent props
    if isempty(props)
       props = struct('channelCount',[uint32(0)],'samplingInterval',[0],'resolutions',[zeros(1,34)],'channelNames',[0]);
    end
